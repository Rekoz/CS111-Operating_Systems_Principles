...
"         -dDIR        Upload and download files from directory DIR.\n"
	...
	if ((m1 = sem_open("sem1", O_CREAT, 0644, 1)) == SEM_FAILED)
	  {
	    perror("semaphore initialization");
	    exit(1);
	  }

	if ((m2 = sem_open("sem2", O_CREAT, 0644, 1)) == SEM_FAILED)
	  {
	    perror("semaphore initialization");
	    exit(1);
	  }

	if ((m3 = sem_open("sem3", O_CREAT, 0644, 1)) == SEM_FAILED)
	  {
	    perror("semaphore initialization");
	    exit(1);
	  }

	if ((m4 = sem_open("sem4", O_CREAT, 0644, 1)) == SEM_FAILED)
	  {
	    perror("semaphore initialization");
	    exit(1);
	  }
